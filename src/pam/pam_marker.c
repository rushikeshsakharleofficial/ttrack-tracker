...
/*
 * Write /run/trackterm-rec/sessions/<sid>.json so the daemon can cross-reference
 * PAM-stamped sessions against connected shims.
 */
int trackterm_pam_write_marker(pam_handle_t *pamh, const char *sid)
{
    char path[512];
    char buf[1024];
    int fd, n;
    ...
    mkdir(TRACKTERM_RUN_DIR,      0755);
    mkdir(TRACKTERM_SESSIONS_DIR, 0755);

    snprintf(path, sizeof(path), "%s/%s.json", TRACKTERM_SESSIONS_DIR, sid);
    ...
    if (fd < 0) {
        pam_syslog(pamh, LOG_WARNING,
                   "trackterm: cannot write marker %s: %s", path, strerror(errno));
        return -1;
    }
...
void trackterm_pam_remove_marker(const char *sid)
{
    char path[512];
    snprintf(path, sizeof(path), "%s/%s.json", TRACKTERM_SESSIONS_DIR, sid);
    unlink(path);
