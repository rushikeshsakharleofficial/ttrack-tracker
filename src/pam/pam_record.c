 ...
 * config file can extend via /etc/trackterm-rec/recd.conf exclude_services,
                                   ...
                                   int argc, const char **argv)
{
    ...
    r = trackterm_pam_chain_env(pamh);
    if (r != PAM_SUCCESS)
        return PAM_SUCCESS; /* fail-open */

    char *sid = trackterm_pam_get_sid(pamh);
    if (sid) {
        trackterm_pam_write_marker(pamh, sid);
        free(sid);
    }
                                    ...
                                    int argc, const char **argv)
{
    ...
    sid_env = pam_getenv(pamh, "TRACKTERM_REC_SID");
    if (sid_env && sid_env[0])
        trackterm_pam_remove_marker(sid_env);
