...
int  trackterm_server_handle_client(void *client, int epoll_fd);
void trackterm_server_drop_client(void *client, int epoll_fd);
...
static int create_dirs(const char *storage_dir)
{
    ...
    if (stat(storage_dir, &st) < 0) {
        if (mkdir(storage_dir, 0750) < 0 && errno != EEXIST) {
            TRACKTERM_LOG_ERR("mkdir %s: %s", storage_dir, strerror(errno));
            return -1;
        }
    }

    snprintf(path, sizeof(path), "%s", TRACKTERM_SESSIONS_DIR);
    if (mkdir(TRACKTERM_RUN_DIR, 0755) < 0 && errno != EEXIST) {}
...
int main(int argc, char *argv[])
{
    ...
    while ((opt = getopt(argc, argv, "c:dh")) != -1) {
        ...
        case 'h':
            fprintf(stderr, "Usage: trackterm-recd [-c config]\n");
            return 0;
        }
    }

    trackterm_config_load(conf_path);
    trackterm_daemon_config_t *cfg = trackterm_config_get();

    trackterm_log_init("trackterm-recd", 0, cfg->log_level);
    ...
    /* Socket activation: systemd may hand us a pre-bound socket fd. */
    {
        ...
        if (n > 0) {
            ...
            TRACKTERM_LOG_INFO("using systemd socket-activated fd %d", listen_fd);
        } else {
            listen_fd = trackterm_server_bind(cfg->socket_path, 0666);
            if (listen_fd < 0) {
                TRACKTERM_LOG_ERR("bind %s: %s", cfg->socket_path, strerror(errno));
                return 1;
            }
        }
    }
    ...
    if (epoll_fd < 0) {
        TRACKTERM_LOG_ERR("epoll_create1: %s", strerror(errno));
        return 1;
    }
    ...
    TRACKTERM_LOG_INFO("trackterm-recd started, socket=%s storage=%s",
    ...
    while (g_running) {
        ...
        for (int i = 0; i < n; i++) {
            if (events[i].data.fd == listen_fd) {
                trackterm_server_accept(listen_fd, cfg->storage_dir, epoll_fd);
            } else {
                void *c = events[i].data.ptr;
                if (!c) continue;

                if (events[i].events & (EPOLLHUP | EPOLLERR)) {
                    trackterm_server_drop_client(c, epoll_fd);
                } else if (events[i].events & EPOLLIN) {
                    trackterm_server_handle_client(c, epoll_fd);
                }
            }
        }
    }

    TRACKTERM_LOG_INFO("trackterm-recd shutting down");
