...
`pmp-rec` records terminal sessions via a PTY-intercept shim and central daemon.
...
```
   sshd / login / su / sudo
       ...
       pam_record.so      ← PAM session_open: mint/chain SID
            │
            ▼
    /etc/profile.d/pmp-rec.sh  (or /etc/zshenv for zsh)
            │ exec
            ▼
       pmp-rec   (PTY shim, runs as user)
            │ AF_UNIX SOCK_STREAM
            ▼
       pmp-recd  (daemon, epoll, root)
            │
            ▼
...
- Sends binary frames over AF_UNIX to `pmp-recd`
...
- Chains: promotes existing `PMP_REC_SID` to `PMP_REC_PARENT`, sets new SID
...
SSH login → PAM: SID=A parent=""
  bash starts → profile.d execs pmp-rec → records session A
  user runs: sudo -i
    sudo PAM: SID=B parent=A
      root bash → profile.d execs pmp-rec → records session B
      root runs: su - alice
        su PAM: SID=C parent=B
          alice bash → records session C
```

`pmp-rec-cli tree <A>` reconstructs and displays this chain.
